...
## 2026-09-17 — 预生成窗口与HTTP查询解耦（0.1.5未发布）

- 恢复项目迁入zr、Qwen/星历0.1.4已发布状态，读取项目AGENTS、文档/context/日志及后端/API、编码、共享包规范。
- warm-astro新增--days（1至31），按日期优先顺序补齐指定星座；单条失败继续、汇总失败后非零退出，已有记录不覆盖。新增maintain-astro常驻命令，默认当天起7天，每轮后等3600秒并按TIMEZONE重新计算日期，补齐失败与新日期。
- HTTP默认ASTRO_GENERATE_ON_REQUEST=false，缓存缺失直接503且不调用模型/等待生成锁；保持现有成功响应和writer读取。后台命令调用原有生成服务，复用事务锁及结构校验；run_local.py隔离测试显式启用请求内生成。无表结构、客户端或模型提示变化。
- 新增预生成运维文档、监管服务模板、切换顺序与历史日期补齐说明；明确任务需单独部署，HTTP不自动排队任意日期。
- 版本0.1.5；71项pytest全部通过（隔离PostgreSQL并发/主库、缓存未命中不调模型、预热后HTTP/重跑缓存、跨日跨年、失败继续、取消关闭session、CLI失败状态）。临时测试容器停止删除，未使用业务数据库/真实模型。
- Black/差异检查通过；poetry构建wheel/sdist，分发检查确认星历及NOTICE完整、无私有.env。未提交、发布PyPI或部署，不修改现有私有.env或生产预生成配置。
...
## 2026-09-18 — 运势正文扩展为详细段落

- 按用户要求更新 Qwen 提示词：summary 100～150字，四项维度各60～100字，presummary保留15～30字；要求具体场景、行动建议、注意事项，以及日/周/月/年的不同侧重点，禁止编造用户经历或新增天象依据。
- max_tokens由2400提高至6500，五期结构和每字段1～300字符硬校验不变，超时继续使用已有配置；原缓存不覆盖。更新docs/qwen-data-contract.md。
- 定向生成/预生成回归41 passed、1 skipped（未配置独立PG测试）；Black检查及git diff --check通过。未调用真实模型，新增文案质量、实际耗时待模型环境验证；未发布、部署或改生产数据，已有dist需重新构建后才能发布新提示词。

## 2026-09-18 — 发布0.1.5准备
